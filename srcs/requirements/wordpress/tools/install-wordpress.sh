...
WP_ADMIN-USER=$(grep WORDPRESS_ADMIN_USER /run/secrets/credentials | cut -d'=' -f2)
...
until mysql -h"${WORDPRESS_DN_HOST%:*}" -u"${WORDPRESS_DB_USER}" -p"${DB_PASSWORD}" -e "SELECT 1" &>/dev/null; do
	sleep 2
done
...
chmod -R www-data:www-data:www-data /var/www/html
